  ...
  while(1) {
    ...
    if (!size && flush_counter==0) {       /* no more instructions (instructions) to simulate */
      ...
      if (control_hazard){
        ...
        memcpy(&pq.instr2, tr_entry, sizeof(pq.instr2));
        IF = get_NOP();
      }
      ...
      if (pq.instr1.PC != 0) {
           // printf("putting pq in if !!! cycle_number is: %d , instruction moved back is: %d", cycle_number, pq.instr1.PC);
            EX = ID;
            ID = IF;
            IF = pq.instr1;
            pq.instr1 = pq.instr2;
          ...
          if (data_hazard) { 
            ...
            if(!size ){    /* if no more instructions in trace, reduce flush_counter */
              ...
              //printf("\nputting tr_entry in IF %d cycle_number is: %d \n",  tr_entry->PC, cycle_number);
